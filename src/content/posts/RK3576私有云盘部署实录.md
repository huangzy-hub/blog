...
pinned: false
description: 用 RK3576、SSD、U 盘和一台公网服务器搭建可挂载为 Windows Z 盘、也能通过域名和 Key 访问的私有云盘。
tags: [RK3576, 私有云, Headscale, Samba, FRP, FileBrowser]
category: 嵌入式
draft: false
---

这篇文章记录一套真正部署并经过公网读写测试的私有云盘：RK3576 负责保存文件，Windows 电脑通过 Headscale/Tailscale 把它挂载为 `Z:` 盘，陌生设备则打开 HTTPS 域名、输入一个访问 Key 后使用网页文件管理器。
...
星号让 Windows 交互输入 Samba 密码，避免明文出现在命令历史。此后 `Z:\SSD` 与 `Z:\USB` 就是两块实际存储设备。
...
Samba 共享根目录跨越两个文件系统，Windows 只能显示共享根对应的一个文件系统统计，不能自然表示 SSD 与 U 盘之和。真实容量以 RK 的 `df -hT /srv/cloud/SSD /srv/cloud/USB` 为准；网页则通过拆分 source 分别显示。
